...
\subsubsection{Build and Run Using Helper Script}

The project includes a helper script that automatically rebuilds the Docker image from scratch and runs the application:

\begin{lstlisting}[language=bash, caption=Run with Helper Script (Recommended)]
# Use provided helper script (Git Bash / WSL)
./run-docker-windows.sh

# The script automatically:
# 1. Builds Docker image from scratch with --no-cache
# 2. Mounts src/main/resources directory
# 3. Runs the climate analysis application
# 4. Writes results to src/main/resources/output/
\end{lstlisting}

\subsubsection{Manual Docker Build and Run}

\begin{lstlisting}[language=bash, caption=Build Docker Image from Scratch]
# Build the Docker image (no cache for clean build)
...
\begin{itemize}
    ...
    \item \textbf{Windows native library errors}: Use Docker (required for Windows) - native PySpark has incompatibility issues
    \item \textbf{Python worker crashed}: This indicates Windows Hadoop library issues - use Docker instead
    \item \textbf{Docker not building}: Ensure Docker Desktop is running and has adequate memory (at least 4GB)
...
\textbf{Challenge:} PySpark on Windows requires Hadoop native libraries (winutils.exe) which cause critical errors including "Python worker exited unexpectedly (crashed)" and "UnsatisfiedLinkError: NativeIO\$Windows.access0". Initial attempts to use \texttt{sc.textFile()} with glob patterns failed due to Hadoop's file system layer requiring native Windows libraries.

\textbf{Solution:} Implemented a dual-mode data loading approach:
\begin{itemize}
    \item For local files matching glob patterns, use Python's \texttt{glob} module and \texttt{open()} to read files directly, then parallelize with \texttt{sc.parallelize()}
    \item For non-local paths, fallback to Spark's \texttt{textFile()} method
    \item Updated README to mandate Docker usage for Windows users
    \item Enhanced \texttt{run-docker-windows.sh} script to automatically rebuild Docker images from scratch using \texttt{--no-cache} flag
\end{itemize}

This solution provides complete Windows compatibility through Docker while maintaining the ability to run natively on Linux/macOS systems.
